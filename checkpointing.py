...
def extract_metrics(file_name, exp_name=None):
    ...
    pattern = (
        "^"
        + ("(.*)" if exp_name is None else re.escape(exp_name))
        + MODEL_FILE_NAME_REGEX_MATCH
    )
...
def get_model_files(checkpoint_path, exp_name=None):
    ...
    pattern = (
        f"^"
        + (".*" if exp_name is None else re.escape(exp_name))
        + MODEL_FILE_NAME_REGEX
    )
    model_files = [
        f
        for f in os.listdir(checkpoint_path)
        if os.path.isfile(os.path.join(checkpoint_path, f)) and re.match(pattern, f)
    ]
...
def get_all_checkpoints(checkpoint_path, exp_name, just_files=False):
    ...
    metrics_dict = {
        f: extract_metrics(f, exp_name) for f in model_files
    }  # {file : (step, test_acc, test_loss)}

    if exp_name is not None:
        statistics = torch.load(
            os.path.join(checkpoint_path, f"{exp_name}.pth"), map_location="cpu"
        )
    else:
        statistics = None
    ...
    all_models = {
        metrics_dict[f][0]: torch.load(
            os.path.join(checkpoint_path, f), map_location="cpu"
        )
...
def get_all_checkpoints_per_trials(
    all_checkpoint_paths, exp_name, just_files=False, verbose=False
):
    ...
    for i, checkpoint_path in enumerate(all_checkpoint_paths):
        ...
        all_models, statistics = get_all_checkpoints(
            checkpoint_path, exp_name, just_files
        )
    ...
    if len(all_statistics) > 0:
        ...
        for key_1 in ["train", "test"]:
            ...
            for key in statistics[key_1].keys():
                all_statistics_dic[key_1][key] = [
                    statistics[key_1][key] for statistics in all_statistics
                ]
        #
...
def get_extrema_performance_steps_per_trials(all_metrics, T_max=None):
    ...
    min_train_loss_steps = [
        steps[losses.index(min_loss)]
        for losses, min_loss, steps in zip(train_losses, min_train_losses, all_steps)
    ]
    ...
    min_test_loss_steps = [
        steps[losses.index(min_loss)]
        for losses, min_loss, steps in zip(test_losses, min_test_losses, all_steps)
    ]
    ...
    max_train_accuracy_steps = [
        steps[accs.index(max_acc)]
        for accs, max_acc, steps in zip(
            train_accuracies, max_train_accuracies, all_steps
        )
    ]
    ...
    max_test_accuracy_steps = [
        steps[accs.index(max_acc)]
        for accs, max_acc, steps in zip(test_accuracies, max_test_accuracies, all_steps)
    ]
