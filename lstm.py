...
class LSTMCell(nn.Module):
    def __init__(self, input_size, hidden_size, bias=True):
        ...
        self.input_gate = nn.Linear(
            combined_dim, hidden_size, bias=bias
        )  # [W_i, U_i] and b_i
    ...
    def forward(self, x, hx):
        ...
        # get all outputs i_t, f_t, o_t, \tilde{c}_t, all [batch_size, hidden_size]
        input = F.sigmoid(self.input_gate(combined))
        forget = F.sigmoid(self.forget_gate(combined))
        output = F.sigmoid(self.output_gate(combined))
        candidate_cell_state = F.tanh(self.candidate_cell(combined))

        # now compute cell state update
        new_cell_state = forget * c + input * candidate_cell_state
        new_hidden_state = output * F.tanh(new_cell_state)
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1, bias=True, dropout=0.0):
        ...
        self.layers = nn.ModuleList(
            [
                LSTMCell(
                    input_size if layer == 0 else hidden_size, hidden_size, bias
                )  # input_size for the first layer, hidden_size for the rest
    ...
    def forward(self, x, hx=None):
        ...
        output = x  # (batch_size, seq_len, input_size) for layer 0 input, (batch_size, seq_len, hidden_size) otherwise
        final_hidden_states = []
        final_cell_states = []

        # Process the input sequence layer by layer
        for layer_idx, cell in enumerate(self.layers):
            h_t = h0[layer_idx]  # (batch_size, hidden_size), h_0^{l}
            c_t = c0[layer_idx]  # (batch_size, hidden_size), c_0^{l}
            layer_outputs = []
            ...
            for t in range(seq_len):
                ...
                # (batch_size, input_size) if layer_idx == 0, (batch_size, hidden_size) otherwise
                x_t = output[:, t, :]

                h_t, c_t = cell(x_t, (h_t, c_t))  # [batch_size, hidden_size]
...
class LSTMLM(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.Embedding(
            vocabulary_size, embedding_size, padding_idx=padding_index
        )
        ...
        if share_embeddings:
            ...
            ), "Embedding size and hidden size must be equal to share embeddings"
    ...
    def forward(self, x, hidden_states=None):
        ...
        # first embed the inputs -- this is of size [batch_size, seq_len, embed_dim]
        x_emb = self.embedding(x)

        # now we pass through the LSTM to get all outputs.
        # outputs: shape [batch_size, seq_len, hidden_size]
        # h, c: each of shape [num_layers, batch_size, hidden_size]
        outputs, (h, c) = self.lstm(x_emb, hidden_states)

        # now get the logits for the outputs -- [batch_size, seq_len, vocab_size]
