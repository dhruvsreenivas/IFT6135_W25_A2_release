...
def get_arithmetic_dataset(
    ...
    assert all(
        o in [2, 3] for o in operation_orders
    ), "operation_orders must be 2, 3 or [2, 3]."

    tokens = [str(i) for i in range(max(p, q))] + [
        operator,
        EQ_TOKEN,
        BOS_TOKEN,
        EOS_TOKEN,
    ...
    if operation_orders == [2, 3]:
        ...
        padded_data = torch.full(
            (len(encoded_data), max_length), tokenizer.stoi[PAD_TOKEN], dtype=torch.long
        )
    ...
    train_data, valid_data = (
        padded_data[:N_train],
        padded_data[-1 if N_train == N_total else N_train :],
    )
    train_mask, valid_mask = (
        mask[:N_train],
        mask[-1 if N_train == N_total else N_train :],
    )
    train_eq_pos, valid_eq_pos = (
        eq_positions[:N_train],
        eq_positions[-1 if N_train == N_total else N_train :],
    )

    # inputs, labels, eq_positions, masks
    train_dataset = TensorDataset(
        train_data[:, :-1], train_data[:, 1:], train_eq_pos, train_mask[:, :-1]
    )
    valid_dataset = TensorDataset(
        valid_data[:, :-1], valid_data[:, 1:], valid_eq_pos, valid_mask[:, :-1]
    )

    return (train_dataset, valid_dataset), tokenizer, max_length, padding_index


def get_arithmetic_dataset_multiple_orders(
    p: int,
    q: int,
    operator: str,
    r_train: float,
    operation_orders: Union[int, List[int]] = [2, 3],
    is_symmetric: bool = False,
    shuffle: bool = True,
    seed: int = 42,
):
    ...
    (dataset, _), tokenizer, max_length, padding_index = get_arithmetic_dataset(
        p, q, operator, 1.0, operation_orders, is_symmetric, shuffle, seed
    )
    dataset_per_order = {
        2: Subset(dataset, [i for i in range(len(dataset)) if dataset[i][2] == 3]),
        3: Subset(dataset, [i for i in range(len(dataset)) if dataset[i][2] == 5]),
    }

    ds_lens_2 = [
        int(r_train * len(dataset_per_order[2])),
        len(dataset_per_order[2]) - int(r_train * len(dataset_per_order[2])),
    ]
    ds_lens_3 = [
        int(r_train * len(dataset_per_order[3])),
        len(dataset_per_order[3]) - int(r_train * len(dataset_per_order[3])),
    ]
    train_ds_2, val_ds_2 = random_split(dataset_per_order[2], ds_lens_2)
    train_ds_3, val_ds_3 = random_split(dataset_per_order[3], ds_lens_3)

    train_dataset = ConcatDataset([train_ds_2, train_ds_3])
    val_dataset = ConcatDataset([val_ds_2, val_ds_3])

    return (train_dataset, val_dataset), tokenizer, max_length, padding_index
